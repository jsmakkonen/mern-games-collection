                ...
                <tbody>
                    {upcomingGames.map(item => (
                        <tr key={item._id}>
                            ...
                            <td> {item.release} </td>
                            <td><EditUpcoming item={item} /></td>
                            <td><button className="btn btn-danger" onClick={() => deleteUpcomingGame(item._id)}>Delete</button></td>
                        </tr>
                    ))}
                </tbody>
            </table>
            <div>
                <Link to='/addupcoming'><button className="btn btn-primary">Add Game to Watchlist</button></Link>
            </div>
        </Fragment>
    )
}
